...
export class Bot extends Client {
    ...
    private notEnoughArgs?: (message: Message, expected: number, got: number) => void;
    private lackOfPermissionsHandler?: (message: Message, command: ICommand) => void;
    ...
    public onNotEnoughArgs(handler: (message: Message, expected: number, got: number) => void): Bot {
    ...
    public lackOfPermissions(fn: (message: Message, command: ICommand) => void): Bot {
    ...
    private handleMessage(message: Message) {
        ...
        if (commandName === "help" && this.helpEnabled) {
            helpService(this.modules, message, this.config, args)
                .catch(console.error);
        } else {
            if (command) {
                this.handleCommand(command, args, message);
            } else {
                if (this.cmdNotFoundHandler) { this.cmdNotFoundHandler(message); }
            }
        }
     ...
     */
    private handleCommand(command: ICommand, args: string[], message: Message): void {
        // Checks if the arguments are correctly passed
        if (command.maxArgs && args.length > command.maxArgs) {
            if (this.notEnoughArgs) { this.notEnoughArgs(message, command.maxArgs, args.length); }
        } else if (command.minArgs && args.length < command.minArgs) {
            if (this.notEnoughArgs) { this.notEnoughArgs(message, command.minArgs, args.length); }
        } else if (command.numArgs && args.length !== command.numArgs) {
            if (this.notEnoughArgs) { this.notEnoughArgs(message, command.numArgs, args.length); }
        } else {
            if (command.permissions) {
                for (const permission of command.permissions) {
                    if (!message.member.hasPermission(permission.toUpperCase() as PermissionResolvable)) {
                        if (this.lackOfPermissionsHandler) { this.lackOfPermissionsHandler(message, command); }
                    } else {
